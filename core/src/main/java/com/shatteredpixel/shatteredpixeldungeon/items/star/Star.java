...
public class Star extends KindofMisc {
	...
	private static ItemStatusHandler<Star> handler;
	...
	public static void initGems() {
		handler = new ItemStatusHandler<>( (Class<? extends Star>[])Generator.Category.RING.classes, gems );
	}
	...
	public static void restore( Bundle bundle ) {
		handler = new ItemStatusHandler<>( (Class<? extends Star>[])Generator.Category.RING.classes, gems, bundle );
	}
	...
	@Override
	public String name() {
		return isKnown() ? super.name() : Messages.get(Star.class, gem);
	}
	...
	public String info(){
		...
		if (cursed && isEquipped( Dungeon.hero )) {
			desc += "\n\n" + Messages.get(Star.class, "cursed_worn");

		} else if (cursed && cursedKnown) {
			desc += "\n\n" + Messages.get(Star.class, "curse_known");

		} else if (!isIdentified() && cursedKnown){
			desc += "\n\n" + Messages.get(Star.class, "not_cursed");
	...
	public static HashSet<Class<? extends Star>> getKnown() {
		return handler.known();
	}
	
	public static HashSet<Class<? extends Star>> getUnknown() {
	...
	public void onHeroGainExp( float levelPercent, Hero hero ){
		...
		if (levelsToID <= 0){
			identify();
			GLog.p( Messages.get(Star.class, "identify", toString()) );
	...
	public int soloBonus(){
		if (cursed){
			return Math.min( 0, Star.this.level()-2 );
		} else {
			return Star.this.level()+1;
		}
	}

	public int soloBuffedBonus(){
		if (cursed){
			return Math.min( 0, Star.this.buffedLvl()-2 );
		} else {
			return Star.this.buffedLvl()+1;
		}
	}
	...
	public class RingBuff extends Buff {
		...
		public int level(){
			return Star.this.soloBonus();
		}

		public int buffedLvl(){
			return Star.this.soloBuffedBonus();
		}
