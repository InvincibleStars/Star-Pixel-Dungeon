...
public class HealthBar extends Component {
	...
	private static final int COLOR_AP = 0xFFBBEEBB;

		private static final int COLOR_BG	= 0xFFCC0000;

		private static final int COLOR_APG	= 0x00000000;
	...
	private ColorBlock Apg;

	private ColorBlock Ap;
	...
	protected void createChildren() {
		...
		Apg = new ColorBlock( 1, 0, COLOR_APG );
		add( Apg );

		Ap = new ColorBlock( 1, 0, COLOR_AP );
		add( Ap );
	...
	protected void layout() {
		...
		Ap.size( width*(attackpower-1), height );
	}
	...
	public void level( float health, float shield, float attackpower ){
		this.health = health;
		this.shield = shield;
	...
	public void level(Char c){
		float health = c.HP;
		float attackpower = c.ATTACKPOWER;
		float shield = c.shielding();
		float max = Math.max(health+shield, c.HT);

		level(health/max, (health+shield)/max,attackpower);
	}
}
